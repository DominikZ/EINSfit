...
#####
#extra packages from standard library
...
#for take_closest_value function
...
################################################################
# Additional functions not needed by EINSfit class
#####

def take_closest_value(myList, myNumber):
    """
...
#####
# END - Additional functions not needed by EINSfit class
################################################################

################################################################
# Additional functions NEEDED by EINSfit class
#####
...
def load_json(input,convert_lists_to_numpy=False):
    ''' Load JSON file
        ...
        Parameters:
        -----------
        input : string = file to load
        convert_lists_to_numpy : bool, optional
            If True, found lists in file will be converted to numpy arrays.
            !! Only works with dictionary file as input !!
...
def save_json(input,name_out):
    ''' Save JSON file
        ...
        Parameters:
        -----------
        input : variable which should be saved as JSON file (e.g. dict)
        name_out : string
...
def load_pickle(input):
    ''' Load pickle file
        ...
        Returns saved variable in pickle. 
    '''
    with open(input, 'rb') as infile:
...
def save_pickle(input,name_out): 
    ''' Save pickle file
        ...
        Parameters:
        -----------
        input : variable which should be saved as pickle file
        name_out : string
            Name of the output file.
    '''
...
def convert_lists_to_numpy_in_dic(dic_in,key=None):
    ''' Converts lists in (nested) dictionary to numpy arrays.
            - !! Changes input dictionary !!
            - !! Only works with dictionary file as input !!
    '''
...
#two Exponential model, NOT implemented in program, but left for curious reader
...
#####
# END - Additional functions NEEDED by EINSfit class
################################################################
...
class EINSfit:
    ...
    def __init__(self,datafile,name=None,data_type='elascan',dic_data_to_use=None,save_dir_path=None):
        ...
        if data_type=='elascan':
            ...
            file_path='Numpy array input with initial name "%s"'%name
        ...
        #sorting of raw data if necessary
        if not is_sorted(raw_T):
            sort_arr=raw_T.argsort()
            raw_T=raw_T[sort_arr]
            raw_DataT_value=raw_DataT_value[sort_arr,:]
            raw_DataT_error=raw_DataT_error[sort_arr,:]
            print('Info: Raw data was sorted in temperature.')
        if not is_sorted(raw_q):
            sort_arr=raw_q.argsort()
            raw_q=raw_q[sort_arr]
            raw_DataT_value=raw_DataT_value[:,sort_arr]
            raw_DataT_error=raw_DataT_error[:,sort_arr]
            print('Info: Raw data was sorted in Q.')
        
        
        #save raw data read and sorted; make sure that data type in arrays is numpy.float which is compatible to python float!
        self._raw_data_type=data_type
        self._raw_file_path=file_path
        self._raw_q=raw_q.astype(float)
        self._raw_data=raw_DataT_value.astype(float)
        self._raw_data_err=raw_DataT_error.astype(float)
        self._raw_T=raw_T.astype(float)
        ...
        self._config_dic=self._define_default_config()
        ...
        if _key in dic:
            ...
            if val:
                if type(_default)!=type(val):
                    #needed in case val was an int instead of float, but still checks for other types, e.g. lists
    ...
    def _define_data_used(self,dic_data_to_use=None):
        assert(not self._used_exists),'Used data already exists'
        if dic_data_to_use is None:
            dic_data_to_use={}
        # .item() makes sure that numpy data type is transformed to python data type (e.g. numpy.float32 to float)
        self._used_Tmin=self._dic_use_value_or_default(dic_data_to_use,'T_start',self._raw_T[0].item())
        self._used_Tmax=self._dic_use_value_or_default(dic_data_to_use,'T_end',self._raw_T[-1].item())
        
        self._used_qmin=self._dic_use_value_or_default(dic_data_to_use,'Q_min',self._raw_q[0].item())
        self._used_qmax=self._dic_use_value_or_default(dic_data_to_use,'Q_max',self._raw_q[-1].item())
        
        #use minimal and maximal Q and T values to define used data
        idx_min=self._raw_T>=self._used_Tmin
        idx_max=self._raw_T<=self._used_Tmax
        idx_T_bool=np.logical_and(idx_min,idx_max)

        idx_min=self._raw_q>=self._used_qmin
        idx_max=self._raw_q<=self._used_qmax
        idx_Q_bool=np.logical_and(idx_min,idx_max)
        
        self._used_T=self._raw_T[idx_T_bool]
        used_data_T=self._raw_data[idx_T_bool,:]
        used_data_T_err=self._raw_data_err[idx_T_bool,:]

        self._used_q=self._raw_q[idx_Q_bool]
        self._used_data=used_data_T[:,idx_Q_bool]
        self._used_data_err=used_data_T_err[:,idx_Q_bool]
        

        #delete specific Ts defined in dic_data_to_use['delete_specific_T-values_list']:
        dic_T_del=self._dic_use_value_or_default(dic_data_to_use,'delete_specific_T-values_list',[])
        for T_del in dic_T_del:
            take_values_idx=np.where(np.invert(np.isclose(T_del,self._used_T)))
            self._used_T=self._used_T[take_values_idx]
            self._used_data=np.take(self._used_data,take_values_idx[0],axis=0)
            self._used_data_err=np.take(self._used_data_err,take_values_idx[0],axis=0)

        #delete specific Qs defined in dic_data_to_use['delete_specific_Q-values_list']:
        ...
        #info
        print('-'*40 + '\nInitializing used data for sample "%s"'%(self._name))
    ...
    @staticmethod
    def _define_default_config() -> dict:
        config_dic={}
        config_dic['Global']={}
        ...
        config_dic['Global']['offset_start_min_max']=[0.9, 0.1, np.exp(config_dic['Global']['GA_intercept_max'])]
    ...
    def read_config_file(self,filename=None,):
        ...
        print('-'*24,'Read config file \'%s\' and following config is updated:'%(filename))
    ...
    def save_config_file(self,record_nb=None,save_path=None,silent=False):
        ...
        result_str=''
        key_order=self._sort_config_dic(dic)
    ...
    ## NOT implemented, idea to also load a saved record
    '''# def _load_saved_record(self):
        ...
        def _load_warnings_of_record_in_nice_results_dic(self) -> list:
            warnings_process=[]
            warnings_options=[]
            try:
    ...
    @staticmethod
    def _sort_config_dic(dic,key_order=None):
    ...
    def plot_results(self,record_nb=-1,save=False,close_all=False,save_path=None,silent=False,outputfile_type='png',outputfile_dpi=200):
        ...
        #get colors to be consistent in all graphs
        #colors order is 0-4: data points, GA model, PK model, Yi model, Do model
        ...
        for nb_fig in range(0,int(len(used_T)/16)+1,1):
            ...
            for i in range(nb_fig*16,nb_fig*16+16,1):
                ...
                ft.set_prop_cycle( cycler( color=colors[0:2] + colors[1:] ) ) #two times same color

                ft.errorbar(used_q_squared, used_data_log[i,:],yerr=used_data_err_log[i,:], marker='o',label='T=%i' % used_T[i])
                ft.errorbar(options_dic['models']['GA']['qValues_squared'], used_data_log[i,options_dic['models']['GA']['qValues_idx']][0],yerr=used_data_err_log[i,options_dic['models']['GA']['qValues_idx']][0], marker='o',ls='None',label='fit region')
                GA_y_fit_temp = lin_modelLmfitMultiBasic(dic_lmfit['GA'][i].params, i, used_q**2)
                ft.plot(used_q**2,GA_y_fit_temp,label='Gaus3 %.2f' % (dic_lmfit['GA'][i].params['slope_%02d' % (i+1)].value*-3),zorder=3)
        ...
        #all fits plot
        fig_fits_allT_list = []
        x_long=np.linspace(0,used_q[-1]+0.2,100)
        for nb_fig in range(0,int(len(used_T)/16)+1,1):
            fig1 = plt.figure(figsize=(20,10))
            nbplots=0
            for i in range(nb_fig*16,nb_fig*16+16,1):
                ...
                if options_dic['use_fit_doster']:
                    #uses also il as index, as defined before
            ...
            plot_title_string='FileIn: %s, Qmin: %.2f$\\AA^{-1}$ Qmax: %f$\\AA^{-1}$, GA Qmin-Qmax: %.2f-%.2f$\\AA^{-1}$, Yi Qmax: %.2f$\\AA^{-1}$, PK Qmax: %.2f$\\AA^{-1}$' %\
                                (self._raw_file_path,used_q[0],used_q[-1],options_dic['models']['GA']['qValues'][0],options_dic['models']['GA']['qValues'][-1]
        ...
        if not save: #only plotted if not saved
            ...
            plt.gca().set_prop_cycle(cycler(color=colors[2:4]))
            yS_Yi,yS_YiErr=[],[]
            yS_PK,yS_PKErr=[],[]
            for i in range(0,len(used_T),1):
    ...
    def get_save_dir(self):
        return str(self._save_dir_path)
    ...
    # @raw_data_type.setter #define how to set variable
    # def raw_data_type(self, value):
    #     self._raw_data_type = value
    # @raw_data_type.deleter
    # def raw_data_type(self) #define how to delete variable
    ...
    @property
    def used_Tmin(self) -> float:
        return self._used_Tmin

    @property
    def used_Tmax(self) -> float:
        return self._used_Tmax

    @property
    def used_qmin(self) -> float:
        return self._used_qmin

    @property
    def used_qmax(self) -> float:
        return self._used_qmax
